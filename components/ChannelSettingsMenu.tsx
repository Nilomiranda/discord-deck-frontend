...
const ChannelSettingsMenu = ({ channel }: ChannelSettingsMenuProps) => {
  ...
  const handleSave = async () => {
    try {
      setSaving(true)
      await saveChannelPresetRoles(channel?.id, selectedRoles?.map(selectedRole => selectedRole?.value));
      toast({
        status: 'success',
      ...
      <>
        <Menu>
          ...
          <MenuList background={"gray.800"}>
            <MenuItem background={'gray.800'} _focus={{ background: 'gray.700' }} _hover={{ background: 'gray.700' }} onClick={handleOpenPresetRolesSettingsModal}>Preset roles</MenuItem>
          </MenuList>
        </Menu>
